    ...
    <div class="min-h-screen flex items-start justify-center px-4 py-10">
        <div class="w-full max-w-6xl">
            ...
            <h1 class="text-2xl font-bold mb-6">
                {{ $title }} <span class="text-sm text-gray-500">(Past 12 Months)</span>
            </h1>
                        ...
                        <tbody class="text-gray-800 divide-y divide-gray-200">
                            @foreach($records as $op)
                                <tr class="hover:bg-gray-50 transition cursor-pointer"
                                    onclick="window.open('https://host.nxt.blackbaud.com/constituent/records/{{ $op->record_id }}','_blank')">
                                    ...
                                    @if($title === 'Target Ask Opportunities')
                                        <td class="px-4 py-2 text-center whitespace-nowrap">
                                            ${{ number_format($op->target_ask, 2) }}
                                        </td>
                                        <td class="px-4 py-2 text-center whitespace-nowrap">
                                            {{ \Carbon\Carbon::parse($op->date_asked)->format('M d, Y') }}
                                        </td>
                                    @elseif($title === 'Expected Opportunities')
                                        <td class="px-4 py-2 text-center whitespace-nowrap">
                                            ${{ number_format($op->amount_expected, 2) }}
                                        </td>
                                        <td class="px-4 py-2 text-center whitespace-nowrap">
                                            {{ \Carbon\Carbon::parse($op->date_expected)->format('M d, Y') }}
                                        </td>
                                    @elseif($title === 'Funded Opportunities')
                                        <td class="px-4 py-2 text-center whitespace-nowrap">
                                            ${{ number_format($op->amount_funded, 2) }}
                                        </td>
                                        <td class="px-4 py-2 text-center whitespace-nowrap">
                                            {{ \Carbon\Carbon::parse($op->date_closed)->format('M d, Y') }}
                                        </td>
                                    @else
